...
GUIA RÁPIDO (o que usar, quando e quando NÃO usar):
  ...
  kd knowledge map   VISÃO de clusters (estrutural/semântico). NÃO use para achar 1 nota.
  kd maintenance ..  SAÚDE (doctor/--audit), índice (index), limpeza (compact/learn/prune).
                     learn/compact/prune só PROPÕEM; watch-service = auto-drain ocioso.
  kd forget <ID>     SOFT-DELETE (--restore; --purge só após retenção).
...
ÂNCORAS (--anchor PATH) — o que liga a memória ao código (alto valor):
  Toda nota/tarefa que fala de um arquivo ou módulo deve ser ancorada:
    ...
    kd task new \"Migrar cache\" --scope task --anchor plan/016.md
...
CONHECIMENTO (kd write — fatos, decisões, erros, riscos, perguntas):
  ...
  kd write --type <fact|decision|error|risk|question> \"<...>\" [--body -|TXT] [--tag T...] [--anchor PATH...]
...
TAREFAS (kd task — epic ⊃ { issue ⊃ task | task }; epic é a raiz, ancore-o):
  ...
  kd task list --ready|--blocked [--explain] [--sort impact] [--kind K] [--tag T]
...
MAPA: kd knowledge map [--axis anchor|type|classification|scope] [--scope C] [--semantic] [--members] [--write]
