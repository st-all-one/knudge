...
| `retrieval` | Índice derivado, BM25, âncoras, filtros, views, RRF, pipeline de hits e vocabulário de tags (`tag_counts`, D107). |
...
| `task` | Hierarquia `plan ⊃ epic ⊃ issue ⊃ task`, ciclo de vida, **espécie** (`--kind`, D113), dono derivado (`claim`, D114), **papel/modo** derivados (D115/D116), **impacto** (`impact`/`is_actionable`, D106/D109), **templates de plano** (`--prompt`/`--from`, D105) e **programas externos** (D119). |
