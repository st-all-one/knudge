...
#[must_use]
pub fn next_tasks(index: &Index, graph: &Graph, limit: usize) -> Vec<NextTask> {
...
pub fn manifest_at(
    ...
    let ready = next_tasks(index, graph, usize::MAX);
    let shown = ready.len().min(limit);
    let mut text = manifest_text(index, graph, changed_paths);
    if shown > 0 {
