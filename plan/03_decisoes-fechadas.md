...
| D163 | **`kd doctor` de topo.** `kd maintenance doctor [--audit]` vira `kd doctor [--fix] [--explain]`: a execução padrão roda os **13 checks + auditoria** num só relatório; `--fix` corrige o reversível; `--explain` detalha cada achado (`esperado`/`encontrado`/`ação`); `--audit` **deixa de existir**. `healthy` só com **zero achados**; advisórios viram `degraded` (nunca "saudável" silencioso). Revisa D119. |
| D164 | **`kd help` de topo.** `kd help` == `kd --help`; `kd help <verbo>` mostra o help embutido do verbo (D167). |
| D165 | **Verbosidade de `init`/`self`/`config`/`sync`/`maintenance`.** Cada um lista o que fez (`novo`/`atualizado`/`inalterado`, caminhos/arquivos) e termina em `próximos:` quando há passo seguinte. |
| D166 | **`prime` compacto por padrão.** `PRIME_COMPACT` (essencial) é o default; `--long` = protocolo completo + gramática TOON. O golden `prime.txt` passa a ser o compacto. |
| D167 | **Help embutido por verbo.** O `--help` de cada verbo ensina, exemplifica e explicita o escopo obrigatório (`--universe`) — a ajuda é conteúdo, não só a lista de flags. |
| D168 | **Redundância da superfície.** `--anchors` (plural) sai (só `--anchor`); o subcomando `body` sai; um só vocabulário por conceito. Revisa D140/D147. |
| D169 | **Documentação viva e estática em sincronia.** `prime`/`help` (viva) e `docs/`/`SKILL.md`/`AGENTS.md`/`llms.txt`/matriz (estática) refletem a superfície; `doctor`/`drain` de topo e `knowledge digest` inexistente. |
