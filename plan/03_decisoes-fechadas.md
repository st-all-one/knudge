...
| D132 | **`kd maintenance watch-service` instala o worker ocioso — com consentimento.** Pergunta no stderr (`s/N`); stdin não-TTY ou `n` cancela (exit 0, sem efeito). Aprovado, baixa `scripts/knudge-idle.sh` na tag `v<versão>` via `curl` (HTTPS) para o staging de cache e o executa (`bash <script> install --project …`), criando o timer `systemd --user`. `--script` usa um arquivo local (offline); `--dry-run` só imprime o plano; `--yes` pula a pergunta. O binário não fala HTTPS nativamente (o adaptador HTTP é só para o `/v1/embeddings` local), por isso delega o download ao `curl` — o mesmo caminho do `install.sh`. |
