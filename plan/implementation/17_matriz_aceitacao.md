...
| `kd maintenance doctor [--audit]` | `ok\|warn\|fail <check> <msg>` por linha | `{checks[], healthy, fixed[]}` (check tem `ok`/`warn`); `--audit` traz `duplicate_pairs`/`broken_anchor_details`/`missing_edge_details`/`stale_lock_details`/`integrity_issues` | `internal` (70) se store corrompido | read-only (com `--fix`, corrige reversível) | `health::tests::doctor`, `cli::regressions_031::audit_json_exposes_duplicate_pairs`, `cli::regressions_031::program_anchor_warn_keeps_corpus_healthy` |
...
| `kd maintenance learn [--scope C] [--tag ...|--anchor ...|--type ...|--class ...|--around ...|--universe]` | `kind\|ids\|score\|why` (inclui `create_note` de tarefa fechada — X2/D111; item com `scope` compara só o `statement`) | `{proposals[]}` | `invalid_input` (2) sem escopo/`--universe` (D144) | read-only | `maintenance::tests::learn`, `cli::d144_maintenance_requires_scope` |
