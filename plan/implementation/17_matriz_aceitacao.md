...
| `kd task list [--ready\|--blocked [--explain]] [--kind K] [--owner A\|--mine]` | `id\|scope\|status\|statement[\|motivo]` | `{tasks[]}` | `invalid_input` (2) se `--ready`+`--blocked`, `--explain` sem `--blocked` ou `--mine` sem `KNUDGE_AGENT` | nada | `retrieval::tests::views::block_reason_*`, `cli::task_list_ready_blocked_and_explain`, `cli::task_kind_sets_type_and_filters`, `cli::task_claim_sets_and_clears_owner` |
