...
| `kd task new [--source F] [--kind K]` | `task_xxx` | `{id, scope, status}` | `invalid_input` (2) se `scope` inválido/profundidade > 4; `schema` (8) se `--kind` incoerente | `notas/<id>.md` (`type` = `scope`/`kind`) + evento | `task::tests::hierarchy`, `task::tests::kind::*` |
| `kd task list [--ready\|--blocked [--explain]] [--sort impact] [--kind K] [--owner A\|--mine]` | `id\|scope\|status\|statement[\|motivo][\|unblocks=N]` | `{tasks[]}` (`impact` por linha com `--sort impact`) | `invalid_input` (2) se `--ready`+`--blocked`, `--explain` sem `--blocked`/`--sort impact` ou `--mine` sem `KNUDGE_AGENT` | nada | `retrieval::tests::views::block_reason_*`, `task::tests::impact::*`, `cli::task_list_ready_blocked_and_explain`, `cli::task_list_sort_impact_orders_critical_path`, `cli::task_list_sort_impact_skips_closed`, `cli::task_kind_sets_type_and_filters`, `cli::task_claim_sets_and_clears_owner` |
