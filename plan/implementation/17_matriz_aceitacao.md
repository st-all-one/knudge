...
| `kd ask` | `id\|statement\|score\|why` (1/linha; `why = semantic` se veio do vetor — D121; `limit` default 5; canal vetorial se `recall.semantic`; `hits[].confidence` inclui confirmação por tarefa — X1/D108) | `{hits[], warnings[]}` | `io` (5) se índice ilegível | nada (read-only) | `retrieval::tests::recall`, `cli::init_write_ask_roundtrip`, `cli::ask_semantic_channel_reads_vector_index`, `cli::task_outcome_promotes_anchored_note_in_ask` |
...
| `kd task new [--source F] [--kind K] [--tag T]` (sem arestas — D126) | `task_xxx` | `{id, scope, status}` | `invalid_input` (2) se `scope` inválido/profundidade > 4; `schema` (8) se `--kind` incoerente | `notas/<id>.md` (`type` = `scope`/`kind`) + evento | `task::tests::hierarchy`, `task::tests::kind::*` |
| `kd task list [--ready\|--blocked [--explain]] [--sort impact] [--kind K] [--tag T] [--anchor P] [--since TS] [--owner A\|--mine]` | `id\|scope\|status\|statement[\|motivo][\|unblocks=N]` | `{tasks[]}` (`impact` por linha com `--sort impact`) | `invalid_input` (2) se `--ready`+`--blocked`, `--explain` sem `--blocked`/`--sort impact`, `--mine` sem `KNUDGE_AGENT` ou `--since` malformado | nada | `retrieval::tests::views::block_reason_*`, `retrieval::tests::views::error_kind_work_item_is_ready_but_knowledge_error_is_not`, `task::tests::impact::*`, `cli::task_list_ready_blocked_and_explain`, `cli::task_list_sort_impact_orders_critical_path`, `cli::task_list_sort_impact_skips_closed`, `cli::task_list_filters_by_tag_anchor_and_since`, `cli::task_kind_sets_type_and_filters`, `cli::task_claim_sets_and_clears_owner` |
...
| `kd task graph [--program <path>\|--root <id>]` | `id\|role\|kind\|status\|owner\|mode\|statement` indentado; containers com ` (done/total)` (D127) | `{program, roots[], nodes[]}` (`progress{done,total}` nos containers) | `not_found` (3) se sem Épico-raiz/raiz | nada (read-only) | `task::tests::program::*`, `task::tests::role::*`, `task::tests::mode::*`, `task::tests::progress::*`, `cli::task_graph_program_renders_subtree`, `cli::task_graph_reports_supervisor_mode` |
| `kd task show <ID> [<ID>...] [--history]` | nota + `pai:`/`bloqueado_por:`/`bloqueia:`/`filhos:`/`epico:` resolvidos (D125/D127), separadas por `\n---\n` | `{tasks[]}` com `parent`/`blocked_by`/`blocks`/`children`/`epic{done,total}` | `not_found`/`io` (3/5) só se **nenhum** id existir; ausente parcial vira `warnings[]` | nada | `task::tests::context::*`, `task::tests::progress::*`, `cli::task_show_multiple_ids_separator_and_partial`, `cli::task_show_includes_context` |
| `kd task update` | `updated\|<id>` | `{id, changed[]}` | `invalid_input` (2) | revisão + evento | `task::tests::lifecycle` |
| `kd task close [--outcome S] [--note TXT]` | `closed\|<id>\|<outcome>\|r<N>` + linha `epico: <id>\|<título> (<done>/<total>)` (D127) | `{id, outcome, evidence[], epic{done,total}}` | `invalid_input` (2) sem evidência ou `--note` sem `--outcome` | grava `outcomes[]`/`evidence` (com `notes`) | `task::tests::lifecycle`, `cli::task_close_note_records_outcome_reason`, `cli::task_close_reports_epic_progress` |
...
| `kd maintenance index --drain` | `indexed=… pending=… stale=… cache_hits=…` | idem | degrada com `warnings[]` se provider offline | atualiza `.idx/embeddings.jsonl`/`emb_cache.jsonl`; **auto-drain ocioso** roda isso ao fim de verbos não-`maintenance` quando `mode=lazy` (D131) | `embeddings::tests::pipeline`, `cli::idle_lazy_drains_queue_after_command`, `cli::idle_manual_mode_leaves_queue_pending`, `cli::eager_mode_is_rejected` |
...
| `kd maintenance watch-service [--yes] [--dry-run]` | resumo do worker | `{installed, script}` ou `{dry_run, source, reference, command}` | `io` (5) se download/execução falhar | instala timer `systemd --user` **após confirmação** (D132) | `cli::watch_service_dry_run_reports_plan`, `cli::watch_service_declined_does_nothing`, `cli::watch_service_runs_local_script` |
| `kd knowledge map [--axis A] [--scope C] [--semantic] [--members]` | `<axis>\|<key>\|<count>` (container: `\|<título>`); membros indentados com `--members`; `semantic\|<axis>\|<key>\|groups=N` | `{docs, clusters[], semantic[]}` | `invalid_input` (2) eixo desconhecido; degrada com `warnings[]` se sem índice de embeddings | read-only (D47) | `lifecycle::tests::clusters::*`, `lifecycle::tests::semantic::semantic_clusters_preserve_parent`, `lifecycle::tests::semantic::complete_link_prevents_chaining`, `cli::knowledge_map_reports_container_axis` |
