...
|---|---|---|---|---|---|
| `kd` (sem args) | help completo (== `kd --help`) (D171) | — | — | nada | `cli::no_args_shows_help`, `cli::help_subcommand_lists_verbs` |
| `kd` + `--json` sem verbo | — | `{success:false,…}` | `invalid_input` (2) | nada | `cli::json_without_verb_is_invalid_input` |
| `kd <verbo>` sem args | help do verbo (exit 0) | — | — | nada | `cli::verb_without_args_shows_help` |
| `kd prime` | protocolo **compacto** estático, byte-idêntico (D166) | `{protocol, version}` | — | nada (read-only) | `golden::prime_matches_golden`, `golden::json_prime_matches_golden`, `cli::json_envelope_for_prime_is_valid` |
...
| `kd init` | `projeto <nome> fundado em <root>/.knudge` + `estrutura:`/arquivos (`novo`/`atualizado`/`inalterado`) + `próximos:` (D165) | `{project, root}` | `config` (7) se já existe sem `--force` | cria `notas/`, `eventos/`, `.idx/`, `cache/`, `config.toml`, `AGENTS.md` | `golden::init_message_matches_golden`, `cli::init_write_ask_roundtrip` |
...
| `kd maintenance compact [--verify]` | `propostas` (merge/supersede); com `--verify`, `...\|gate=passed\|failed`; `propostas: <kind>=N` + `próximos:` (D165) | `{proposals[]}` (com `gate` no `--verify`) | `invalid_input` (2) sem escopo/`--universe` (D144) | read-only (propõe; nada sem aceite; `--verify` é read-only — D156) | `maintenance::tests::compact`, `cli::d144_maintenance_requires_scope` |
...
| `kd maintenance learn [--verify] [--scope C] [--tag ...|--anchor ...|--type ...|--class ...|--around ...|--universe]` | `kind\|ids\|score\|why` (inclui `create_note` de tarefa fechada — X2/D111; item com `scope` compara só o `statement`); com `--verify`, `...\|gate=passed\|failed`; `propostas: <kind>=N` + `próximos:` (D165) | `{proposals[]}` (com `gate` no `--verify`) | `invalid_input` (2) sem escopo/`--universe` (D144) | read-only (D47; `--verify` é read-only — D156) | `maintenance::tests::learn`, `cli::d144_maintenance_requires_scope` |
| `kd maintenance prune [--scope C] [--tag ...|--anchor ...|--type ...|--class ...|--around ...|--universe]` | `forget\|id\|motivo` (1/linha) + `propostas: forget=N` + `próximos:` (D165) | `{proposals[]}` | `invalid_input` (2) sem escopo/`--universe` (D144) | read-only (propõe; aplica só com `kd forget`) | `cli::maintenance_prune_proposes_forget_for_expired`, `cli::d144_maintenance_requires_scope` |
| `kd maintenance watch-service [--install\|--subscribe\|--unsubscribe\|--status\|--uninstall] [--yes] [--dry-run]` | saúde ou resumo da ação + `próximos:` (D165) | `{action, done, script}` ou `{dry_run, action, source, reference, command}` | `io` (5) se pré-flight/execução falhar; uso (2) se flags de ação conflitarem | instala/gerencia o agendador (`systemd --user`/`launchd`) e o servidor de embeddings persistente; multi-projeto; mutar exige confirmação (D132/D133) | `cli::watch_service_dry_run_reports_plan`, `cli::watch_service_defaults_to_status`, `cli::watch_service_action_flags_are_exclusive`, `cli::watch_service_declined_does_nothing`, `cli::watch_service_subscribe_runs_local_script` |
...
| `kd config get` | `chave = valor (projeto\|global) — <path>` (D165) | `{key, value, scope}` | `not_found` (3) se chave ausente | nada | `cli::*`, `config::tests` |
| `kd config set` | `chave = valor (projeto\|global) — <path>` (D165) | `{key, value, scope}` | `config` (7) se chave desconhecida | grava `.knudge/config.toml` (ou global) | `config::toml::tests` |
| `kd config unset` | `chave removida (projeto\|global) — <path>` / `chave já ausente …` | `{key, scope}` | `not_found` (3) | remove override | `config::tests` |
| `kd config list` | `projeto (N) — <path>` + `chave = valor` por linha (D165) | `{entries[]}` | — | nada | `config::tests` |
...
| `kd sync` | `sync <branch>: N arquivo(s) commitados (<hash curto>)`; `nada a sincronizar` (D165) | `{committed, message, files[], branch, hash}` | `io` (5)/`config` (7) fora do git | commit de `notas/`+`eventos/` | `git::tests::sync` |
...
| `kd` == `kd --help`; `--json` sem verbo é uso (2) | `cli::no_args_shows_help`, `cli::json_without_verb_is_invalid_input`, `golden::prime_matches_golden` |
